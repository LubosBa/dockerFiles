#!/usr/bin/env bash
# This is an entrypoint of Docker Kafka container, it will setup some
...
# Check, if Kafka config folder is empty:
if [[ -z "$(ls -A /config/)" ]]; then
    cp -r /opt/kafka_2.13-2.6.0/config/* /config/
fi
...
/opt/kafka_2.13-2.6.0/bin/kafka-server-start.sh /config/kafka-server.properties